...
if st.session_state["authentication_status"]:
    ...
    with tab1:
        ...
        with st.form("add_book", clear_on_submit=True):
            ...
            book_format = st.selectbox("Format", ["NA", "Audiobook", "Hardcover", "Paperback", "pdf"])
            ...
            if submitted and title and author:
                ...
                conn.execute(
                    "INSERT INTO books (id, username, title, author, format, start_date, end_date, isbn) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
                    [next_id, st.session_state["username"], title, author, book_format, start_date, end_date, isbn]
                )
                conn.close()
    ...
    with tab2:
        ...
        conn = get_connection()
        books = conn.execute("SELECT id, title FROM books WHERE username = ? AND end_date IS NOT NULL", [st.session_state["username"]]).fetchall()
        conn.close()
        ...
        if books:
            with st.form("add_review", clear_on_submit=True):
                ...
                if submitted:
                    ...
                    conn.execute(
                        "INSERT INTO reviews (id, username, book_id, form, function, comment) VALUES (?, ?, ?, ?, ?, ?)",
                        [next_id, st.session_state["username"], book_choice[0], form_score, function_score, comment]
                    )
                    conn.close()
        ...
        df_reviews = conn.execute("""
            ...
            WHERE r.username = ? AND b.username = ?
        """, [st.session_state["username"], st.session_state["username"]]).fetchdf()
        conn.close()
        ...
        conn = get_connection()
        df_books = conn.execute("SELECT title, end_date FROM books WHERE username = ? AND end_date IS NOT NULL", [st.session_state["username"]]).fetchdf()
        conn.close()
    ...
    with tab3:
            ...
            if book_choice:
                ...
                if st.button("💾 Save Changes"):
                    ...
                    conn.execute(
                        ...
                        [new_format, new_start, new_end, new_isbn, book_id, st.session_state["username"]],
                    )
                    conn.close()
                ...
                if st.button("🗑️ Delete Book"):
                    conn = get_connection()
                    conn.execute("DELETE FROM reviews WHERE book_id=? AND username=?", [book_id, st.session_state["username"]])
                    conn.execute("DELETE FROM books WHERE id=? AND username=?", [book_id, st.session_state["username"]])
                    conn.close()
        ...
        with st.expander("Delete ALL Books"):
            ...
            if st.button("🗑️ Delete ALL Books") and confirm:
                conn = get_connection()
                conn.execute("DELETE FROM reviews WHERE username=?", [st.session_state["username"]])
                conn.execute("DELETE FROM books WHERE username=?", [st.session_state["username"]])
                conn.close()
